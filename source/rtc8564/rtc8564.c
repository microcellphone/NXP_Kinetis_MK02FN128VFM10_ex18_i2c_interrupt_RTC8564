...
// Get RTC Data
//====================
void RTC8564_Get_Current_Time_Request(RTC_INFO *rtc)
{
  rtc->year = INT_BCD(RTC8564_Read_Reg(RTC_YEARS));
  rtc->mon  = INT_BCD(RTC8564_Read_Reg(RTC_C_MONTHS) & 0x1f);
  rtc->day  = INT_BCD(RTC8564_Read_Reg(RTC_DAYS) & 0x3f);
  rtc->week = RTC8564_Read_Reg(RTC_WEEKDAYS) & 0x07;
  rtc->hour = INT_BCD(RTC8564_Read_Reg(RTC_HOURS) & 0x3f);
  rtc->min  = INT_BCD(RTC8564_Read_Reg(RTC_MINUTES) & 0x7f);
  rtc->sec  = INT_BCD(RTC8564_Read_Reg(RTC_SECONDS) & 0x7f);
}
